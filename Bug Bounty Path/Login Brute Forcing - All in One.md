...
Last                    Key
```
First, install **ruby**, and then pull the **Username Anarchy** git for the script:
```bash
...
```
Upon inspecting ``jane_smith_usernames.txt``, you'll encounter a diverse array of usernames:
...
The efficacy of ``CUPP`` hinges on the quality and depth of the information you feed it. It's akin to a detective piecing together a suspect's profile. So, where can one gather this valuable intelligence for a target like **Jane Smith**?
...
* **News Articles and Blogs**: News articles or blog posts could shed light on **her interests, achievements, or affiliations**.
...
![kuva](https://hackmd.io/_uploads/SkQ6lzlr1l.png)
...
This process results in a highly personalized wordlist, significantly more likely to contain Jane's actual password than any generic, off-the-shelf dictionary could ever hope to achieve.

We can install CUPP using:
```bash
...
```
We now have a generated ``username.txt`` list and ``jane.txt`` password list, but there is one more thing we need to deal with.

**CUPP** has generated many possible passwords, but Jane's company, AHI, has a rather odd password policy:
...
Now we would use the two generated lists in **Hydra** against the target to brute-force the login form:
```bash
...
```
Once Hydra has completed the attack, we can log into the website using the discovered credentials.
